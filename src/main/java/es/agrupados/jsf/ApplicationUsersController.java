...
/**
 * Class with CRUD methods for ApplicationUserController.
 * @author mundakamacbook
 */
@Named("applicationUsersController")
@SessionScoped
public class ApplicationUsersController implements Serializable {

    @EJB
    private es.agrupados.beans.ApplicationUsersFacade ejbFacade;
    private List<ApplicationUsers> items = null;
    private ApplicationUsers selected;

    public ApplicationUsersController() {
    }

    public ApplicationUsers getSelected() {
        return selected;
    }

    public void setSelected(ApplicationUsers selected) {
    ...
    public ApplicationUsers prepareCreate() {
        selected = new ApplicationUsers();
        initializeEmbeddableKey();
        return selected;
    }

    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/Bundle").getString("ApplicationUsersCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/Bundle").getString("ApplicationUsersUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/Bundle").getString("ApplicationUsersDeleted"));
        if (!JsfUtil.isValidationFailed()) {
